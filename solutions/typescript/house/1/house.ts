...
export function verse(stanzaNumber: number) {
  switch(stanzaNumber) {
    case 1:
...
export function verses(startStanza: number, endStanza: number) {
  const resultPoem: string[] = []

  for (let i = startStanza; i <= endStanza; i++) {
    resultPoem.push(...verse(i))
