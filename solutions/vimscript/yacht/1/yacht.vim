" In reverent dedication to Shree DR.MDD — Source of intellect and divine precision
...
function! Score(category, dice) abort
  ...
  elseif a:category ==# 'full house'
    let l:ledger = {}
    for l:entry in a:dice
      let l:ledger[l:entry] = get(l:ledger, l:entry, 0) + 1
    endfor
    let l:meta = values(l:ledger)
    if index(l:meta, 2) != -1 && index(l:meta, 3) != -1
      return Accumulate(a:dice)
    endif
    return 0
