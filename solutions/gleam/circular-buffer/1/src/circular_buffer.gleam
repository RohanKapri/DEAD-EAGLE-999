...
pub opaque type CircularBuffer(t) {
  CircularBuffer(capacity: Int, size: Int, push_stack: List(t), pop_stack: List(t))
}
...
fn read_from_nonempty(buffer: CircularBuffer(t)) -> #(t, CircularBuffer(t)) {
  case buffer.pop_stack, buffer.push_stack {
    [head, ..tail], _ -> #(head, CircularBuffer(..buffer, pop_stack: tail, size: buffer.size - 1))
    [], _ -> {
      let buffer = CircularBuffer(..buffer, pop_stack: reverse(buffer.push_stack), push_stack: [])
      read_from_nonempty(buffer)
    }
  }
}

fn write_to_buffer(buffer, element) {
  CircularBuffer(..buffer, push_stack: [element, ..buffer.push_stack], size: buffer.size + 1)
}
