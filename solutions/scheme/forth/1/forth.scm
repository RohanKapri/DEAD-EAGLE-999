...
(define (evaluate-statement statement environment stack)
  (let walk ((statement statement) (stack stack))
    (cond ((null? statement) stack) ;; return stack
          ((number? (car statement)) (walk (cdr statement) (cons (car statement) stack)))
...
(define (define-forth atoms environment)
  (assert (not (number? (car atoms))))
  `((,(car atoms) ,@(fold-right (lambda (atom definition)
...
(define (parse-forth source)
  (define (parse-statement statement)
    ...
    (define (parse-definition stmt)
      `(define-forth
         ,@(parse-statement (substring stmt 1 (- (string-length stmt) 1)))))
    (define (parse-line stmt)
      (with-input-from-string (string-downcase stmt)
        (lambda ()
