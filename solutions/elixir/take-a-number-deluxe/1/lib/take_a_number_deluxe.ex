defmodule TakeANumberDeluxe do
  ...
  defp do_init([{:min_number, number}|rest],map) do
    do_init(rest,%{map|min: number})
  end
  defp do_init([{:max_number, number}|rest],map) do
    do_init(rest,%{map|max: number})
  end
  defp do_init([{:auto_shutdown_timeout, timeout}|rest], map) do
    do_init(rest,%{map|timeout: timeout})
  end
  ...
  @impl true
  def handle_cast(:reset_state,
    %{min_number: min, max_number: max,
      auto_shutdown_timeout: timeout}) do
    case State.new(min,max,timeout) do
      {:ok,state} -> 
        send_timeout(state)
        {:noreply,state}
  ...
  def handle_info(_message,state) do
    #IO.inspect(_message, label: "message")
    {:noreply, state}
  end
end